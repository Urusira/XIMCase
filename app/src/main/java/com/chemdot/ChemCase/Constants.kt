...
object Constants {
    ...
    fun getQuestions(typeQ: String): ArrayList<Question>{
        ...
        val nonmutablefilteredQuests : List<Question>
        if (typeQ == "All") {
            nonmutablefilteredQuests = getQuests
        } else {
            nonmutablefilteredQuests = (  getQuests.filter { it.type == "$typeQ" })   /*  Фильтрую вопросы по типу. typeQ - переменная, которую мы передаём текущей функции при её вызове*/
        }
    ...
    fun getQuests(): ArrayList<Question>{
        ...
        questions.add(Question(
            ...
            "Прежде всего, следует попытаться оба пятна удалить органическим растворителем " +
                    ...
                    " Ткань следует слегка подогреть, например утюгом или на батарее отопления, - это ускорит процесс возгонки"       //   Пояснение к верному ответу
        ))
        idishnik++
        questions.add(Question(
            idishnik,                                                           //  Айдишник вопроса
